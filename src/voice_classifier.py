...
class VoiceClassifier:
    ...
    def prepare_model(self):
        ...
        self.X_test = X_test
        self.model = self.make_model(self.x_train.shape[1:], num_classes)
    ...
    def create_classiffication_report(self):
        cm = confusion_matrix(self.model_to_clf(self.x_test), self.y_test)
        cm_data = pd.DataFrame(cm, columns=[self.speaker_list[0], self.speaker_list[1]])
        cm_data.index = [self.speaker_list[0], self.speaker_list[1]]
        sns.heatmap(cm_data.astype(int), fmt='d', annot=True,cmap='plasma')
        plt.yticks(rotation=0)
        plt.xticks(rotation=0,fontsize=10)
        plt.show()
        clf_report = classification_report(self.y_test,
                                   self.model_to_clf(self.x_test),
                                   labels=[0, 1, 2],
                                   target_names=[self.speaker_list[0], self.speaker_list[1]],
                                   output_dict=True)
        sns.heatmap(pd.DataFrame(clf_report).iloc[:-1, :].T, annot=True,cmap='plasma')
        plt.show()
    
    def model_to_clf(self, input_data):
        pred_list = self.model.predict(input_data)
        LIST_PRED = []
        for p in pred_list:
